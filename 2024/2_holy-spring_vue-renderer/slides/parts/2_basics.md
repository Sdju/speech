...
<!--
И как мы понимаем это может быть
- DOM-ом, так и, например
- строкой для SSR
- каким-то рендером в canvas
- рендером pdf-документа
- или даже docx документ
-->
...
И важный момент связан с тем, что мы получаем в template ref это и есть тот самый элемент внешнего мира, те Host-среды.
...
<!--
Давайте еще разберем небольшой пример
...
<!--
Теперь мы хотим использовать наш рендерер, вместо дефолтного вьюшного
...
Поэтому не особо откладываем начинаем их изучение

Правда если мы сейчас запустим проект, то будет пустая страничка и примерно вот такая ошибка.

Это значит, что все хорошо и наш рендерер подцепился и уже пытается брать методы рендерера. Но так как они еще не реализованы, то выкидывается лишь наша заглушка.
-->
